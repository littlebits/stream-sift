...
var errors = {
  ...
  ENOF: {
    code: 'ENOF',
    messageFormat: 'The function %j is not defined.'
  },
  EPATTERNTYPE: {
    code: 'EPATTERNTYPE',
    messageFormat: 'Invalid pattern type. Pattern must either be a PlainObject or a match-all wildcard ("*"), but actual pattern given was %j.'
  }
}
...
exports.assertPatternType = function assertPatternType(pattern) {
  if (isInvalidPatternType(pattern)) throw exports.patternType(pattern)
}

exports.patternType = function patternType(pattern) {
  return createError({
...
var INDENT_SIZE = 2
var keyRegExp = new RegExp('(^\\' + symbols.func + '+)?(.*)')
